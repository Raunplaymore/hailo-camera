...
| `SESSION_LABEL_MAP` | classId→label 매핑 (`0:person,1:player_ready,2:player_not_ready,3:golf_ball,4:club_head,5:club,6:club_handle` 또는 JSON) |
| `HAILO_MODEL_NAME` / `HAILO_MODEL` | 세션 요청에서 허용할 Hailo 모델명 (default `yolov8s`) |
| `HAILO_MODEL_ALIASES` | 추가 허용 모델명 CSV. 기존 클라이언트 호환을 위해 `yolov8s`는 항상 허용 |
...
`golf_swingfeed_yolov8n_service7_960` 모델을 HEF로 변환한 뒤에는 아래처럼 실행 환경을 맞춥니다.

```bash
sudo cp yolov8n_service7_960.hef /usr/share/hailo-models/

export HAILO_MODEL_NAME=yolov8n_service7
